...
# Load project config if present (set by first-time setup)
...
# First-time setup: project name, optional custom ports, fresh git (run once only)
if [ ! -f ".startup-config" ]; then
    echo ""
    echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
    ...
    echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
    echo ""
    DEFAULT_PROJECT_NAME=$(basename "$SCRIPT_DIR" | tr '[:upper:]' '[:lower:]' | tr -cs '[:alnum:]' '-' | sed 's/^-//;s/-$//')
    [ -z "$DEFAULT_PROJECT_NAME" ] && DEFAULT_PROJECT_NAME="my-app"
    echo -e -n "  ${BLUE}Project name${NC} (e.g. my-app) [${CYAN}$DEFAULT_PROJECT_NAME${NC}]: "
    read -r INPUT_NAME
    PROJECT_NAME="${INPUT_NAME:-$DEFAULT_PROJECT_NAME}"
    PROJECT_NAME=$(echo "$PROJECT_NAME" | tr '[:upper:]' '[:lower:]' | tr -cs '[:alnum:]' '-' | sed 's/^-//;s/-$//')
    [ -z "$PROJECT_NAME" ] && PROJECT_NAME="my-app"
    echo ""
    echo -e -n "  ${BLUE}Use default ports?${NC} (y/n) [${CYAN}y${NC}]: "
    read -r USE_DEFAULT
    USE_DEFAULT_PORTS="${USE_DEFAULT:-y}"
    if [ "$USE_DEFAULT_PORTS" != "y" ] && [ "$USE_DEFAULT_PORTS" != "Y" ]; then
        echo -e "  Enter custom ports (press Enter to keep default):"
        echo -e -n "    Supabase API port [${CYAN}58421${NC}]: "
        read -r SUPABASE_API_PORT
        SUPABASE_API_PORT=${SUPABASE_API_PORT:-58421}
        echo -e -n "    Backend port [${CYAN}18000${NC}]: "
        read -r BACKEND_PORT
        BACKEND_PORT=${BACKEND_PORT:-18000}
        echo -e -n "    Frontend port [${CYAN}13000${NC}]: "
    ...
    echo ""
    # Write config for future runs
    {
        echo "# Generated by first-time setup. Do not edit by hand if using start.sh."
        echo "PROJECT_NAME=\"$PROJECT_NAME\""
        echo "USE_DEFAULT_PORTS=\"$USE_DEFAULT_PORTS\""
    ...
    # Update supabase config with project name
    if [ -f "supabase/config.toml" ] && command -v sed >/dev/null 2>&1; then
        _sed_cmd="sed -i.bak"
        [[ "$OSTYPE" == "linux-gnu"* ]] && _sed_cmd="sed -i"
        $_sed_cmd "s/^project_id = \".*\"/project_id = \"$PROJECT_NAME\"/" "supabase/config.toml" 2>/dev/null || true
        rm -f supabase/config.toml.bak 2>/dev/null || true
    fi
    if [ "$USE_DEFAULT_PORTS" != "yes" ] && [ -f "supabase/config.toml" ] && command -v sed >/dev/null 2>&1; then
        _sed_cmd="sed -i.bak"
        [[ "$OSTYPE" == "linux-gnu"* ]] && _sed_cmd="sed -i"
        $_sed_cmd "/^\[api\]$/,/^\[/ s/^port = [0-9]*$/port = $SUPABASE_API_PORT/" "supabase/config.toml" 2>/dev/null || true
        rm -f supabase/config.toml.bak 2>/dev/null || true
    fi
    # Fresh git repository for the new project
    echo -e "  ${BLUE}Setting up fresh git repository...${NC}"
    rm -rf .git
    git init
    print_success "First-time setup complete. Project: $PROJECT_NAME"
    echo ""
fi

# Re-load config so port overrides from .startup-config apply
if [ -f ".startup-config" ]; then
    # shellcheck source=/dev/null
    source ".startup-config"
fi
# Apply custom ports from config to script variables when set
if [ "$USE_DEFAULT_PORTS" != "yes" ] && [ -n "${SUPABASE_API_PORT:-}" ]; then
    SUPABASE_API_PORT=$SUPABASE_API_PORT
fi
if [ "$USE_DEFAULT_PORTS" != "yes" ] && [ -n "${BACKEND_PORT:-}" ]; then
    BACKEND_PORT=$BACKEND_PORT
fi
if [ "$USE_DEFAULT_PORTS" != "yes" ] && [ -n "${FRONTEND_PORT:-}" ]; then
...
stop_all_services() {
    ...
    if [ -f "./stop.sh" ]; then
        ...
        if command_exists supabase; then
            supabase stop --project-id "$PROJECT_NAME" > /dev/null 2>&1 || true
...
# Start Supabase
print_status "Starting Supabase for project: $PROJECT_NAME (this may take a minute on first run)..."
...
# Check if virtual environment exists (.venv only)
if [ ! -d ".venv" ]; then
